...
void verificar_input(char ch);
bool verificar_numero_existente_g(void);
bool verificar_numero_existente_user(void);
void limpar_numero(void);
...
load_board(void)
{
    ...
    if (fread(g.board, 81 * INTSIZE, 1, fp) != 1 && fread(user.board, 81 * INTSIZE, 1, fp) != 1)
    {
        fclose(fp);
        return false;
    }
...
// Altera o cursor no tabuleiro dependendo da tecla que foi precionada
void move_cursor(int posx, int posy)
{
    if((g.y + posy <= 8 && g.y + posy >= 0) &&
       (g.x + posx <= 8 && g.x + posx >= 0))
    {
        g.x = g.x + posx;
        g.y = g.y + posy; 
    }
...
// Vefifica se o input do usuario eh um numero
void verificar_input(char ch)
{
    if (ch >= '1' && ch <= '9')
    {
        if(!verificar_numero_existente_g() && !verificar_numero_existente_user())
        {
            mvaddch(g.top + g.y + 1 + g.y/3, g.left + 2 + 2*(g.x + g.x/3), ch);
            user.board[g.y][g.x] = ch;
        }
    }
}
...
bool verificar_numero_existente_g(void)
{
    return (g.board[g.y][g.x] != 0) ? true : false;
}

bool verificar_numero_existente_user(void)
{
    return (user.board[g.y][g.x] != 0) ? true : false;
}

void limpar_numero(void)
{
    if(!verificar_numero_existente_g())
    {
        user.board[g.y][g.x] = 0;
